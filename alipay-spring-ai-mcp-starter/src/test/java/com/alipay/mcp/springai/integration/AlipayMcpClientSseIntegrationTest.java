...
/**
 * AlipayMcpClient SSE 模式集成测试
 *
 * 使用支付宝 MCP SSE 端点，验证完整的 MCP 握手和工具调用流程。
 *
 * 运行方式：
 *   ALIPAY_APP_ID=你的应用ID ALIPAY_PRIVATE_KEY=你的私钥 \
 *   mvn test -Dtest="AlipayMcpClientSseIntegrationTest"
 *
 * 可选环境变量：
 *   ALIPAY_SSE_ENDPOINT - SSE 端点 URL（默认为线上地址）
 *
...
class AlipayMcpClientSseIntegrationTest {
    ...
    @BeforeEach
    void setUp() {
        System.out.println("=== SSE 集成测试：初始化 AlipayMcpClient ===");
